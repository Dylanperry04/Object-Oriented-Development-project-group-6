...
public class CsvReader {
    ...
    public static List<Payslip> readPayslips(String fileName) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            String[] columns = line.split(",");
            if (columns.length < 5) { // Ensure at least 5 columns are present
                System.err.println("Skipping malformed line: " + line + " (Columns found: " + columns.length + ")");
                continue;
            }
            ...
            try {
                ...
                // Handle optional HoursWorked column
                int hoursWorked = (columns.length > 5 && !columns[5].isEmpty()) ? Integer.parseInt(columns[5].trim()) : 0;

                // Add to the list of Payslips
